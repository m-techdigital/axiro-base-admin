...
const statusLabels = {
    awaiting_counterparty: 'Chờ Bên B',
    awaiting_party_acceptance: 'Chờ hai bên xác nhận',
    terms_pending: 'Đang thỏa thuận',
    admin_review: 'Chờ thẩm định',
...
export default function EscrowBoxList() {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (value) => <Tag>{value || 'Chưa đánh giá'}</Tag>,
        },
        {
            title: 'Trạng thái',
            dataIndex: 'status',
        ...
        },
        {
            ...
            render: (_, record) => (
                <BaseIconAction
                    icon={<EyeOutlined />}
                    label="Xem box"
                    onClick={() => navigate(`/escrow-boxes/${record.id}`)}
                />
            ),
        },
    ]
        ...
        <BaseListView
            ...
            header={
                <BasePageHeader
                    ...
                    description="Thẩm định điều khoản, phí, rủi ro và điều phối bàn giao với danh tính nội bộ được bảo vệ."
                    actions={
                        <BaseButton
                            type="primary"
                            onClick={() => navigate('/escrow-boxes/new')}
                        >
                            Tạo box cho 2 khách hàng
                        </BaseButton>
                    }
                />
            }
            loading={list.loading}
            onChange={(pagination) =>
                query.paginate(pagination.current, pagination.pageSize)
            }
            pagination={{
