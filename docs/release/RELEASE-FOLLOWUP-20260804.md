...
Admin BaseForm was re-based on the reviewed AXIRO parent foundation instead of maintaining a Mini-specific render/validation engine. Source parity checks pass for the parent registry, Laravel field errors, first-error focus, submit policy, cancel veto, relation cascade and controlled Form.Item behavior. Frontend lint/build/browser verification remains pending until dependencies are installed in the Git working tree.
