...
- Admin CRUD follows AXIRO parent base ownership patterns without importing parent RBAC, company, project, accounting or report dependencies.
- Product, Customer and Transaction forms use `BaseFormPage`, `BaseForm` and `BaseFormFooter` instead of ad hoc page/card/button layouts.
...
- Extended Mini `BaseFormPage` to accept `cardProps`.
...
- Run `npm run format:check`, `npm run check:source-closure`, `npm run lint` and `npm run build`.
